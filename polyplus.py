...
class PolyplusIntegrator(PolyfitBase):
    ...
    def __call__(self, x):
        """
        return the DDP definite integral from 0 to x.
        the return value is DDP if x is DDP.
        """
        q = to_ddp(x)
        ret = zero()
...
## }}}
## {{{ DDP root finding using bisection
...
class PolyplusQuadrature(PolyfitBase):
    ...
    def __init__(self, data):
        ...
        self.x0, self.x1 = to_ddp(data["x0"]), to_ddp(data["x1"])
