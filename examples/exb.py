...
def demo():
    ...
    sc = 1.
    xv = tl.qvec(x * sc for x in range(N))
    yv = [pv(x) for x in xv]
    ...
    for i, c in enumerate(ev.coefs(tl.p.to_quad(N >> 1), -1)):
        print("%2d %s" % (i, tl.format_list(c)))
    ## values at zero and N>>2, expect (N>>1)**D+pi and pi
    print("polyfit vals:")
    for i in (0, N >> 1):
        print("%6d %s" % (i, tl.format_list(ev(tl.p.to_quad(i)))))
    ...
    print("numpy vals:")
    for i in (0, N >> 1):
        print("%6d %s" % (i, tl.format_list(tl.qeval(i, cofs))))
