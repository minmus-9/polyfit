...
def cholesky_decomposition(A):
    ...
    A = [[to_ddp(z) for z in row] for row in A]
    ...
    for i in range(n):
        for k in range(i + 1):
            ...
            if i == k:
                L[i][k] = sqrt(sub(to_ddp(A[i][i]), suma))
            else:
                L[i][k] = mul(div(uno, L[k][k]), sub(A[i][k], suma))
    return L


def cholesky_forward_sub(L, b):
    "fwd sub on a lower triangular matrix"
    y = [to_ddp(z) for z in b]
    n = len(L)
    for i in range(n):
        for j in range(i):
...
def chofit(xv, yv, wv, D):
    "cholesky least-squares fit, DDP"
