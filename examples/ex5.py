...
def demo():
    ...
    exp = tl.qeval(tl.p.to_quad(N >> 1), cvee)
    ...
    print(" P(N>>1)", tl.format_list(ev(tl.p.to_quad(N >> 1))))
    ...
    print("  coefs ", tl.format_list(obs))
    print("  relerr", tl.format_list(rel))
    print("    P(0)", tl.format_list(tl.qeval(tl.p.zero(), cof)))
    print(" P(N>>1)", tl.format_list(tl.qeval(tl.p.to_quad(N >> 1), cof)))
    print("     exp", tl.format_list(exp))
